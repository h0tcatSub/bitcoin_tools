...
def kill_line(line, kill_at_height, pid):
    ...
    if "UpdateTip: new best=" in line:
        try:
            ...
            "    Block height {} read".format(block_height)
            if block_height == kill_at_height:
                kill(pid, SIGTERM)
                "Process with pid {} KILLED!!!".format(pid)
                exit()
...
def follow(thefile, kill_at_height, pid, start_at=START_AT_ORIGIN):
    ...
    while True:
        ...
        if line and not kill_line(line, kill_at_height, pid):
            continue
        if not line:
            "Waiting {} for new data...".format(SLEEP_INT)
            sleep(SLEEP_INT)
            continue
...
if __name__ == '__main__':
    try:
        ...
        "Starting to monitor file {} at {}".format(log_filename, start)
        "I'm going to kill process with pid {} at height {}".format(pid, kill_at_height)

        logfile = open(log_filename, "r")
        follow(logfile, kill_at_height, pid, start_at=start)

    except IOError as err:
        "Bitcoind log file {} not found".format(log_filename)
