...
class TX:
    ...
    def display(self):
        ...
        "version: " + str(self.version) + " (" + change_endianness(int2bytes(self.version, 4)) + ")"
        "number of inputs: " + str(self.inputs) + " (" + encode_varint(self.inputs) + ")"
        for i in range(self.inputs):
            "input " + str(i)
            "\t previous txid (little endian): " + self.prev_tx_id[i] + \
                  " (" + change_endianness(self.prev_tx_id[i]) + ")"
            "\t previous tx output (little endian): " + str(self.prev_out_index[i]) + \
                  " (" + change_endianness(int2bytes(self.prev_out_index[i], 4)) + ")"
            "\t input script (scriptSig) length: " + str(self.scriptSig_len[i]) \
                  + " (" + encode_varint((self.scriptSig_len[i])) + ")"
            "\t input script (scriptSig): " + self.scriptSig[i].content
            "\t decoded scriptSig: " + Script.deserialize(self.scriptSig[i].content)
            if self.scriptSig[i].type is "P2SH":
                "\t \t decoded redeemScript: " + InputScript.deserialize(self.scriptSig[i].get_element(-1)[1:-1])
            "\t nSequence: " + str(self.nSequence[i]) + " (" + int2bytes(self.nSequence[i], 4) + ")"
        "number of outputs: " + str(self.outputs) + " (" + encode_varint(self.outputs) + ")"
        for i in range(self.outputs):
            "output " + str(i)
            "\t Satoshis to be spent (little endian): " + str(self.value[i]) + \
                  " (" + change_endianness(int2bytes(self.value[i], 8)) + ")"
            "\t output script (scriptPubKey) length: " + str(self.scriptPubKey_len[i]) \
                  + " (" + encode_varint(self.scriptPubKey_len[i]) + ")"
            "\t output script (scriptPubKey): " + self.scriptPubKey[i].content
            "\t decoded scriptPubKey: " + Script.deserialize(self.scriptPubKey[i].content)

        "nLockTime: " + str(self.nLockTime) + " (" + int2bytes(self.nLockTime, 4) + ")"
